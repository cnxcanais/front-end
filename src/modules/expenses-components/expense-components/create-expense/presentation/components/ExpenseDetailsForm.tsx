 ...
 * @description A form component for handling expense details including total amount, installment parts, and bank account selection.
 *
 ...
 * @state {number} detailsInfo.totalAmount - Total amount of the expense
...
export function ExpenseDetailForm({
  ...
  const detailsArray = watch("expenseDetailsArray")
  ...
  const renderDetailsInfo = () => {
    return Array(detailsInfo.partsQty)
      .fill(null)
      .map((_, index) => <ExpenseDetailsInfo key={index} index={index} />)
  }
  ...
  useEffect(() => {
    if (detailsArray?.length && bankAccountId) {
      detailsArray.forEach((item, index) => {
        setValue(
          `expenseDetailsArray.${index}.bank_account_id`,
          bankAccountId,
          {
            shouldValidate: true,
          }
  ...
  useEffect(() => {
    if (detailsArray?.length) {
      detailsArray.forEach((item, index) => {
        setValue(`expenseDetailsArray.${index}.part`, index + 1)
        setValue(`expenseDetailsArray.${index}.account_id`, account_id)
        setValue(
          `expenseDetailsArray.${index}.due_date`,
  ...
  useEffect(() => {
    if (initialIndividualValue) {
      detailsArray.forEach((_, index) => {
        setValue(`expenseDetailsArray.${index}.amount`, initialIndividualValue)
      })
    }
  ...
  useEffect(() => {
    ...
    if (
      ...
      const firstAmount = getValues("expenseDetailsArray.0.amount")
      const adjustment = detailsInfo.totalAmount - totalAmount

      setValue("expenseDetailsArray.0.amount", firstAmount + adjustment)
    ...
    if (detailsArray.length === 1) {
      setValue("expenseDetailsArray.0.amount", detailsInfo.totalAmount)
    ...
    <>
      <div className="flex gap-4">
        <div className="flex flex-col gap-4">
          <label className="text-md" htmlFor="expense_total_amount">
            Valor Total
          </label>
          <Input.Root>
            <Input.Currency
        ...
        <div className="flex min-w-[500px] flex-col gap-4">
          <label htmlFor="expense_bank_account_id">Conta Bancária</label>
          <Input.Root
            variant={
              errors?.expenseDetailsArray?.[0]?.bank_account_id ?
                "error"
              : "primary"
            }>
            <Input.SelectInput
              name={`expenseDetailsArray`}
              options={bankAccounts}
              disabled={!expense_input_fields_bank_account_id}
          ...
          {errors.expenseDetailsArray && (
            <span className="mt-1 text-xs text-red-500">
              {errors.expenseDetailsArray[0]?.bank_account_id.message}
            </span>
          )}
        </div>
      </div>
