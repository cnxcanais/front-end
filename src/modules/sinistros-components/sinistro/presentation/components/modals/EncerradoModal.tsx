...
export function EncerradoModal({
  open,
  onClose,
  onConfirm,
  sinistroId,
  ...
  const [formData, setFormData] = useState<Encerrado>({
    observacao: "",
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await changeSinistroStatus(sinistroId, {
        ...formData,
        statusNovo: SinistroStatusEnum.ENCERRADO,
      })
    ...
    <Modal
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="rounded-lg bg-indigo-50 p-3 text-sm text-indigo-800">
          <strong>Objetivo:</strong> Encerrar Sinistro
        </div>
